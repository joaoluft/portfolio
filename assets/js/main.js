$(document).ready(function () {
    ...
    const loadCurrentProjects = (projects) => {
        ...
        projects.forEach((v, k) => {
            $(".projectsCarrousel").append(`
            <a target="_blank" href="./assets/documents/projects/${v.index}.pdf" class="relative" id="card-${k}">
                <div class="absolute bottom-0 left-0 right-0 top-0 grid place-items-center z-30">
                <div>
                    <h1 class="text-white text-center text-xl md:text-6xl font-bold">${v.name}</h1>
                ...
                <div class="relative aspect-auto overflow-hidden">
                    ...
                    <img class="object-cover w-full h-[90vw] md:h-[35vw] z-10" src="${v.thumbnail}" alt="Projeto - ${v.name}">
                </div>
            </a>`);
        });
    };
    ...
    // Projects Carrousel
    $('.projectsCarrousel').slick({
        prevArrow: $('.projectsCarrouselPrevious'),
        nextArrow: $('.projectsCarrouselNext'),
        asNavFor: '.projectsCarrouselNav',
        autoplay: true,
        speed: 500,
    });

    $('.projectsCarrouselNav').slick({
        asNavFor: '.projectsCarrousel',
    });
});